		...
		for(int i=0;i<items.size();i++){
			...
			if(fi.isFormField()){//如果是表单字段	
				...
				// getName()返回文件名称，如果是空字符串，说明没有选择文件。 // FilenameUtils.getName(filename);
					//img=new File(application.getRealPath("/")
						//+System.getProperty("file.separator") +FilenameUtils.getName(dfi.getName())).getAbsolutePath();
					//out.print("文件被上传到服务上的实际位置："+img +"<br>");
					//dfi.write(new File(application.getRealPath("/") + System.getProperty("file.separator") + FilenameUtils.getName(dfi.getName())));
						String url="C:\\Users\\Administrator\\javaworkspace\\jsp3_2\\WebContent\\image";						
						img="image/"+dfi.getName();
						//img=dfi.getName();
						dfi.write(new File(url+"\\"+dfi.getName()));
				}
			}
		}
	}	
