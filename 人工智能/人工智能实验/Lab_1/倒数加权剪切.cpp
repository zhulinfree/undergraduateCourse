...
bool cmp(weight_and_No a,weight_and_No b){
...
int main() {
	...
	for (int i = 10; i < 20; i++) { //每个测试文本
		cout<<"测试文本"<<i-10<<": ";
		float dis[100];//训练文本和测试文本的对应欧氏距离，以及距离的倒数
		weight_and_No weight[100];
		int NoTexts=0;
		//float num=0;
//		float max_weight1=0,max_weght2=0;
		for (int j = 0; j < 10; j++) { //各个训练文本
			...
			weight[NoTexts].weight=1.0/dis[NoTexts];
			weight[NoTexts].No=NoTexts;
			NoTexts++;
		}
		sort(weight,weight+NoTexts,cmp);
		float num=0;
		int cut=2;//取权重最大的前cut个进行归一化加权
		for(int z=0;z<cut;z++){//取前三个最大权重的作为总体权重
			num+=weight[z].weight;
		}
		for(int z=0;z<cut;z++){
			weight[z].weight/=num;
		...
		//算出感动值
		for(int z=0;z<cut;z++){
			preMoved[i]+=weight[z].weight*moved[(weight[z].No)];
		}
		//对于每个欧式距离的权重进行归一化
//		cout<<"weight: ";
...
//			cout<<preMoved[i]<<" ";
//			preMoved[i]+=weight[c].weight*moved[c];
//		}
//		cout<<endl;
