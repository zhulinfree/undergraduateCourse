...
void change();
int ID3(nestVec data, StrIntMap is_used);
...
void build(Node * &root, nestVec data, StrIntMap is_used) {
	...
	int select_col = ID3(data, is_used);
	if (select_col == -1) {
		root = new Node;
...
void printTree(Node * root){
	if(root->child.size()==0){
		...
		cout<<root->s<<endl;
		for(vector<string>::iterator it=node[root->s].begin();it!=node[root->s].end();it++){
			cout<<*it<<"-";
